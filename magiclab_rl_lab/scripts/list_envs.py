...
All the environments are registered in the `magiclab_rl_lab` extension. They start
with `Magiclab` in their name.
"""
...
def import_packages():
    sys.path.insert(0, f"{pathlib.Path(__file__).parent.parent}/source/magiclab_rl_lab/magiclab_rl_lab/tasks/")
...
def main():
    """Print all environments registered in `magiclab_rl_lab` extension."""
    # print all the available environments
    table = PrettyTable(["S. No.", "Task Name", "Entry Point", "Config"])
    table.title = "Available Environments in Magiclab RL Lab"
    ...
    for task_spec in gym.registry.values():
        if ("Magiclab" in task_spec.id  or "Magic" in task_spec.id) and "Isaac" not in task_spec.id:
